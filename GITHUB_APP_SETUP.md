...
## Grounded coding-agent draft across chats and models

Install **GitHub** and **Superpowers**, connect GitHub, and open an authorized repository. From any chat, type `/agent` (optionally followed by your coding request), or use **Start coding task** in either plugin's Manage panel. The agent uses whichever **text/code AI model is selected in the website's model selector**. The agent reads real repository listings, asks the selected model to choose at most three source files, reads their complete contents under bounded size limits, and asks the same model for structured complete-file replacements. The full before/after source is shown in a review panel. Only after the user explicitly presses **Stage reviewed files for PR** do those edits enter the existing multi-file preview and separately signed GitHub PR approval flow. The model cannot commit, alter main, or dispatch GitHub Actions merely by generating text. GitHub does not receive a write request during AI drafting.

Repository file contents needed for the coding task are sent to the **selected AI model provider**. Keep sensitive files and secrets out of the inspected source; the agent excludes sensitive-looking paths and enforces per-file input limits, but cannot guarantee arbitrary source never contains secrets. The model may fail to return usable structured code; the user can narrow the task or select a different supported text/code model. Each task remains in the active browser tab: closing it stops the task; no background autonomous job or shared multi-device task history is provided.

This workflow implements grounded **inspect → plan → draft → user review → PR proposal**. It does **not** yet implement the entire proprietary ChatGPT Codex runtime: no isolated interactive coding sandbox, unlimited repository traversal, arbitrary command execution, unattended test/fix loops, persistent background agent queue, or automatic merge. For actual test execution after creating a PR, use the existing user-approved GitHub Actions panel and verify the returned run conclusions. A passing repository regression suite is not a substitute for testing an actual GitHub App installation and production AI provider connectivity.
