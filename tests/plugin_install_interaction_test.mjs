...
assert.equal(get('jdplugInstalledStrip').children.length,2,'installed icons should appear in horizontal strip');
assert.equal(get('jdplugPreviewStage').hidden,false,'interactive demo should be visible');
get('jdplugDetailMore').click();
assert.equal(get('jdplugDetailMenu').hidden,false,'plugin overflow menu should open');
get('jdplugDetailMenuManage').click();
assert.equal(get('jdplugManageView').hidden,false,'Manage should navigate to actual settings');
get('jdplugSettingsClose').click();
assert.equal(get('jdplugDetail').hidden,false,'settings back button should return to the plugin detail');
